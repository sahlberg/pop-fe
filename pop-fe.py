...
def create_psp(dest, disc_ids, real_disc_ids, game_title, icon0, pic0, pic1, cue_files, real_cue_files, img_files, mem_cards, aea_files, subdir = './', snd0=None, no_pstitleimg=False, watermark=False, subchannels=[], manual=None, use_cdda=False, logo=None, no_libcrypt=None, psx_undither=None, force_ntsc=False, cdda=False, swap_disc=False):
    EMPTY_CONFIG = bytes([
        0x70,0x00,0x07,0x06,0x00,0x00,0x06,0x06,                 
                                                0x00,0x00,0x00,0x00,
                                                                   0xFF,0xFF,0xFF,0xFF,
    ...
    for i in range(len(disc_ids)):
        ...
        if cdda:
            configs[i] = bytearray(configs[i])
            # Set CDDA bit in configs   is this byte offset right os should it be 0x0a ?
        ...
        if swap_disc:
            print('Enable Swapping Discs in config')
            configs[i] = bytearray(configs[i])
            # Set Allow Swap Disc bit in configs
            if len(configs[i]) > 0x09:
                configs[i][0x09] |= 0x80
            if len(configs[i]) > 0x8d:
                configs[i][0x8d] |= 0x80
